...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input, lengths, hidden):
        ...
        packed = nn.utils.rnn.pack_padded_sequence(input, lengths, batch_first=True) # https://www.jianshu.com/p/efe045c24a93
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        return output, hn, torch.zeros([input.size(0), 1, input.size(1)])  # batchSize × 1 * seq_len
...
class BahdanauAttentionDecoderRNN(nn.Module):
    ...
    def forward(self, inputStep, hidden, encoderOutput):
        ...
        #TODO 计算attention的权重部分，attention的本质是softmax   attentionWeight: batchSize * 1 * seq_len    encoderOutput: batchSize * seq_len * hiddenSize    hidden[-1:]: 1 × batchSize × hiddenSize
...
#定义LuongAttention
class LuongAttention(nn.Module): # https://blog.csdn.net/u010960155/article/details/82853632
    ...
    def dot_score(self, hidden, encoderOutput):
        ...
        return torch.sum(hidden * encoderOutput, dim=2) # batchSize × seq_len
...
class Seq2Seq:
    def __init__(self, dataClass, featureSize, hiddenSize, encoderNumLayers=1, decoderNumLayers=1, attnType='L', attnMethod='dot', dropout=0.1, encoderBidirectional=False, outputSize=None, embedding=None, device=torch.device("cpu")):
        ...
        embedding = embedding if embedding else nn.Embedding(outputSize+1, featureSize) # TODO +1?
        #数据读入
    ...
    def train(self, batchSize, isDataEnhance=False, dataEnhanceRatio=0.2, epoch=100, stopRound=10, lr=0.001, betas=(0.9, 0.99), eps=1e-08, weight_decay=0, teacherForcingRatio=0.5):
        ...
        for e in range(epoch):
            for i in range(itersPerEpoch):
                ...
                #计算bleu的参考指标
                if (e*itersPerEpoch+i+1)%stopRound==0: # TODO
    ...
    def save(self, path):
        torch.save({
        	"encoder":self.encoderRNN, 
        	"decoder":self.decoderRNN, 
            "word2id":self.dataClass.word2id, 
            "id2word":self.dataClass.id2word}, path)
...
class ChatBot:
    ...
    def predictByGreedySearch(self, inputSeq, maxAnswerLength=32, showAttention=False, figsize=(12,6)):
        ...
        encoderOutput, hidden = self.encoderRNN(X, XLens, hidden) # output: batchSize × seq_len × hiddenSize; hn: numLayers*d × batchSize × hiddenSize 
        hidden = hidden[-d*self.decoderRNN.numLayers::2].contiguous() # TODO hn: encodernumLayers*d × batchSize × hiddenSize -> decodernumLayers × batchSize × hiddenSize
        ...
        while decoderInput.item() != eosToken and len(Y) < maxAnswerLength:   #确定输出的序列，同时使用attention计算权重，选取最优解
            decoderOutput, hidden, decoderAttentionWeight = self.decoderRNN(decoderInput, hidden, encoderOutput)    # decoderOutput: batchSize(1) × 1 × outputSize
    ...
    def predictByBeamSearch(self, inputSeq, beamWidth=10, maxAnswerLength=32, alpha=0.7, isRandomChoose=False, allRandomChoose=False, improve=True, showInfo=False):
        ...
        encoderOutput = encoderOutput.expand(beamWidth, -1, -1) # => TODO beamWidth × 1 × hiddenSize
        
        for i in range(1, maxAnswerLength): # beamWidth相当于batchSize
            # decoderOutput: beamWidth × 1 × outputSize; 
            # hidden: numLayers × beamWidth × hiddenSize; 
            # decoderAttentionWeight: beamWidth × 1 × XSeqLen
            decoderOutput, hidden, decoderAttentionWeight = self.decoderRNN(decoderInput, hidden, encoderOutput)
            # topv: beamWidth × 1; 
            # topi: beamWidth × 1 
            if improve: #TODO
                decoderOutput = decoderOutput.view(-1, 1) # (beamWidth×outputSize, 1)
                if allRandomChoose:
